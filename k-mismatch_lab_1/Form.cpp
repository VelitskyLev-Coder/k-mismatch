...
std::vector<Form> Form::generateAllForms(uint64_t length, uint64_t matches)
{
    ...
    std::function<void(uint64_t, uint64_t, uint64_t)> generateForms;
    generateForms = [&allForms, &generateForms](uint64_t remainOnes, uint64_t remainZeros, uint64_t curFormInt) {
        if (!remainOnes)
